...
class Appointment_admin extends NodCMS_Controller
{
    ...
    function __construct(){
        ...
        // Stable Provider menu for single provider users
        if($this->_website_info["appointment_multiowner"] != 1
            ...
            $default_provider = $this->Appointment_admin_model->getProviders(array("default"=>1));
         ...
         *      III. staff
         */
        $this->data['justOneProvider'] = FALSE;
        $adminPages = array(
            'providers',
            'providerEdit',
            'providerManipulate',
                ...
                if(count($provider)!=0){
                    ...
                    if(count($providerManager)==0 && in_array($this->session->userdata('group'), array(1, 100))){
                        ...
                        if($this->session->userdata('group')==1 && $providerManager[0]['group_id']!=1){
                            ...
                            $myProviders = $this->Appointment_admin_model->getProviderManagers(NULL, $where);
                            if(count($myProviders)==1){
                                $this->data['justOneProvider'] = TRUE;
                            }
    ...
    function index($id=NULL)
    {
        ...
        if($this->session->userdata('group') == 1 || $this->session->userdata('group') == 100){
            ...
            // Last Users
            $this->data['users'] = $this->Appointment_admin_model->getAllUser(5, 0);
            // Providers List
            $this->data['providers'] = $this->Appointment_admin_model->getProviders(NULL, 5, 0);
    ...
    function providers($page = 1)
    {
        ...
        $config['total_rows'] = count($this->Appointment_admin_model->getProviders());
        $config['uri_segment'] = 4;
        $config['per_page'] = 10;
        $this->mkPagination($config);
        $page--;

        $this->data['data'] = $this->Appointment_admin_model->getProviders(NULL, $config['per_page'], $page*$config['per_page']);
    ...
    function providerEdit($id=null)
    {
        ...
        $this->data['breadcrumb']=array(
            array('title'=>_l("Providers", $this), 'url'=>APPOINTMENT_ADMIN_URL.'providers'),
    ...
    // Provider managers edit page
    function providerManager($id)
    {
        $provider = $this->Appointment_admin_model->getProviders(array('provider_id'=>$id));
        if(count($provider)!=0){
    ...
    function providerManagerNotificationEmail($provider_id){
        if($this->input->raw_input_stream){
            $this->load->library('form_validation');
//            $this->form_validation->set_rules('provider_id', _l('Provider data',$this), 'required|is_natural');
...
//                $provider_id = $this->input->post("provider_id", TRUE);
    ...
    function providerManipulate($id = null)
    {
        ...
        if (in_array($this->session->userdata['group'], array(1, 20))) {
            if($this->input->raw_input_stream){
                $this->load->library('form_validation');
                $this->form_validation->set_rules('data[provider_name]', _l('Provider Name',$this), 'required');
                $this->form_validation->set_rules('data[provider_username]', _l('Provider Unique Name',$this), 'required|callback_validateProviderUsername['.$id.']');
                if ($this->form_validation->run() != TRUE){
                    $result = array("status"=>"error","error"=>validation_errors(),"static_error");
                }else{
...
//                    if($setData["paypal_signature"] == '') unset($setData["paypal_signature"]);
    ...
    // Provider manager for provider admin
    ...
    function services()
    {
        ...
        $this->data['sub_title']=_l("Services list",$this);
        $this->data['breadcrumb']=array(
            array('title'=>$this->data['sub_title'])
        );
    ...
    function serviceEdit($id=null)
    {
        ...
        $this->data['title']=_l("Services",$this);
        $this->data['breadcrumb']=array(
            array('title'=>_l('Services',$this),'url'=>APPOINTMENT_ADMIN_URL.'services'),
            array('title'=>$this->data['sub_title'])
        );
