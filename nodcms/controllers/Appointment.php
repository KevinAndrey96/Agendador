...
class Appointment extends NodCMS_Controller {
    ...
    function multiProviderSystem($method, $lang, $provider_username, $id = NULL){
        ...
        if(count($provider)!=0 && ($this->_website_info['appointment_multiowner'] || $provider[0]['default']!=0)){
            // # Provider exists and (multi provider system = TRUE or the provider default = 1)
            $this->provider_prefix = '/provider/'.$provider[0]['provider_username'];
            $this->provider = $provider[0];
        }else{
            // Provider didn't exists
...
//    Homepage
    ...
    {
        $this->preset($lang);
        $count = $this->Appointment_model->countProviders();
        if($count!=0){
            if(!$this->_website_info['appointment_multiowner']){
                $default_provider = $this->Appointment_model->getAllProviders(array('default'=>1));
    ...
    // Provider list to choose a provider
    private function providerList($lang, $page=1){
        $this->preset($lang);
        // Provider search
        ...
        $config['total_rows'] = $this->Appointment_model->countProviders($searchConditions);
        $config['uri_segment'] = 2;
        $config['per_page'] = 20;
        $this->mkPagination($config);
        $page--;

        $this->data['data_list'] = $this->Appointment_model->getAllProviders($searchConditions, $config['per_page'], $config['per_page']*$page);
...
//                $this->data['data_list'][$key]['service_description'] = 'test';
        ...
        $this->data['title'] = _l('Provider Page', $this);
    ...
    function getService($lang,$id)
    {
        ...
        if(isset($this->data['data']["service_id"])){
            ...
            if(count($this->data['allow_days'])!=0){
                // Success
                $data = array('status'=>'success','disable_dates'=>$this->data['disable_dates'],'allow_days'=>$this->data['allow_days']);
            }else{
                // Error
                $data = array('status'=>'error','error'=>_l("Unfortunately, now is no time available for appointments for this service!",$this));
            }
        }else{
            // Error
...
//    Set an appointment request from compact page with ajax
    function setAppointment($lang,$id)
    {
        $this->preset($lang);
    ...
    function checkAvailableAppointment($lang, $id){
        $this->preset($lang);
    ...
    private function setAppointmentValidation($service_id, $post_data){
        ...
        if(!$duplicate){
            ...
            if($this->Appointment_model->checkFreeDate($intDate,$service_id) == 0){
                ...
                $time_period =  @reset($this->Appointment_model->getTimePeriod($dayNO,$service_id,$thisMin));
                // Check period exists
...
//    Service detail pages without ajax  *(Not work yet)
    function serviceDetail($lang,$id)
    {
        $this->preset($lang);
        $this->data['data'] =  @reset($this->Appointment_model->getServiceDetail($id));
        if(!isset($this->data['data']["service_id"])){
...
//        $this->data['user_data']=$this->Nodcms_general_model->get_user_detail($this->data['data']["user_id"]);
    ...
    function reservationConfirmation($lang, $id)
    {
        $this->preset($lang);
...
//    Get list of available times for services with ajax
    function getTime($lang,$service_id)
    {
        $this->preset($lang);
